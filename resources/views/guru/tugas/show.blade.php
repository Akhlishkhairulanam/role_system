    ...
    <div class="py-4">
        ...
        <div class="card border-0 shadow-sm rounded-4">
            <div class="card-header bg-white py-3 border-0 d-flex justify-content-between align-items-center">
                ...
                <span class="badge bg-primary rounded-pill">{{ count($submissions) }} / {{ count($students) }}
                    Mengumpulkan</span>
            </div>
                            ...
                            @forelse($students as $index => $student)
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        @if ($sub)
                                            <span
                                                class="badge bg-success bg-opacity-10 text-success rounded-pill px-3">Sudah</span>
                                        @else
                                            <span
                                                class="badge bg-danger bg-opacity-10 text-danger rounded-pill px-3">Belum</span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="text-center">
                                        <span
                                            class="fw-bold fs-5 {{ ($sub->grade ?? 0) < 70 ? 'text-danger' : 'text-primary' }}">
                                            {{ $sub->grade ?? '-' }}
                                        </span>
                                    </td>
                                    ...
                                    <td>
                                        @if ($sub && $sub->file_path)
                                            <a href="{{ route('guru.tugas.download_submission', $sub->id) }}"
                                                target="_blank"
                                                class="btn btn-xs btn-link p-0 text-decoration-none fw-bold">
                                                <i class="fas fa-file-download me-1"></i> Unduh
                                            </a>
                                            <div class="small {{ $sub->created_at > $assignment->deadline ? 'text-danger' : 'text-muted' }}"
                                    ...
                                    <td class="text-center">
                                        @if ($sub)
                                            <button class="btn btn-sm btn-primary rounded-pill px-3" data-bs-toggle="modal"
                                                data-bs-target="#modalNilai{{ $sub->id }}">
                                                <i class="fas fa-edit me-1"></i> Nilai
                                            </button>

                                            {{-- MODAL INPUT NILAI --}}
                                            <div class="modal fade" id="modalNilai{{ $sub->id }}" tabindex="-1"
                                                aria-hidden="true">
                                                <div class="modal-dialog modal-dialog-centered">
                                                    <div class="modal-content border-0 shadow rounded-4 text-start">
                                                        <form action="{{ route('guru.tugas.grade', $sub->id) }}"
                                                            method="POST">
                                                            @csrf
                                                            <div class="modal-header bg-light border-0">
                                                                <h5 class="modal-title fw-bold">Input Nilai:
                                                            ...
                                                            <div class="modal-body p-4">
                                                                ...
                                                                <div class="mb-3 text-center bg-light p-3 rounded-3">
                                                                    <label class="small text-muted d-block mb-1">Catatan
                                                                        Siswa:</label>
                                                                    <span
                                                                        class="fst-italic">"{{ $sub->notes ?? 'Tidak ada catatan' }}"</span>
                                                                </div>

                                                                {{-- Input Nilai --}}
                                                                <div class="mb-3">
                                                                    <label class="form-label fw-bold small">Nilai
                                                                        (0-100)</label>
                                                                    <input type="number" name="grade"
                                                                        class="form-control form-control-lg text-center fw-bold text-primary"
                                                                        value="{{ $sub->grade }}" min="0"
                                                                        max="100" required>
                                                                </div>

                                                                {{-- Input Feedback (Opsional, pastikan kolom feedback ada di DB) --}}
                                                                <div class="mb-0">
                                                                    <label class="form-label fw-bold small">Feedback /
                                                                        Komentar Guru</label>
                                        ...
                                        @else
                                            <button
                                                class="btn btn-sm btn-light disabled rounded-pill px-3">Menunggu</button>
                                        @endif
                                    </td>
                                </tr>
                            @empty
