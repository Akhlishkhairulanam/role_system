...
@section('content')
    <div class="py-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <button class="btn btn-primary rounded-pill shadow-sm px-4" data-bs-toggle="modal"
                                ...
                                <tr class="teacher-row">
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <div class="avatar-circle bg-primary bg-opacity-10 text-primary me-3 d-flex align-items-center justify-content-center rounded-circle"
                                            ...
                                            <div>
                                                ...
                                                <small class="text-muted">
                                                    {{ $t->jenis_kelamin }} •
                                                    {{ \Carbon\Carbon::parse($t->tanggal_lahir)->age }} Thn
                                                </small>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        <div class="text-dark small"><i class="far fa-envelope me-1 text-muted"></i>
                                            {{ $t->email }}</div>
                                        <div class="text-muted small"><i class="fas fa-phone me-1 text-muted"></i>
                                            {{ $t->nomor_telepon }}</div>
                                    </td>
                                    <td>
                                        @php
    ...
    elseif ($t->status == 'Non-Aktif')
        $badgeClass = 'bg-secondary bg-opacity-10 text-secondary';
    elseif ($t->status == 'Cuti')
        $badgeClass = 'bg-info bg-opacity-10 text-info';
    elseif ($t->status == 'Pensiun')
        $badgeClass = 'bg-warning bg-opacity-10 text-warning';
    elseif ($t->status == 'Mutasi')
        $badgeClass = 'bg-primary bg-opacity-10 text-primary';
    elseif ($t->status == 'Pindah')
        $badgeClass = 'bg-dark text-white'; // HITAM SOLID (Kontras)
                                        @endphp
                                    ...
                                    <td>
                                        @if($t->user && $t->user->is_active)
                                            <span class="badge bg-success bg-opacity-10 text-success"><i
                                                    class="fas fa-check me-1"></i> On</span>
                                        @else
                                            <span class="badge bg-danger bg-opacity-10 text-danger"><i
                                                    class="fas fa-times me-1"></i> Off</span>
                                        @endif
                                    </td>
                                    <td class="text-end px-4">
                                        <div class="btn-group">
                                            ...
                                            <button class="btn btn-sm btn-light text-warning border-0 rounded-circle me-1"
                                                data-bs-toggle="modal" data-bs-target="#editTeacherModal{{ $t->id }}"
                                                title="Edit Lengkap">
                                            ...
                                            @if(Auth::id() != $t->user_id)
                                                ...
                                                <form id="delete-teacher-{{ $t->id }}"
                                                    action="{{ route('guru.data.teachers.destroy', $t->id) }}" method="POST"
                                                    class="d-none">
                                                    @csrf @method('DELETE')
                                                </form>
                                            @endif
                    ...
                    <div class="modal-body p-4">
                        <div class="row g-4">
                            <div class="col-md-4 text-center border-end">
                                <div class="avatar-lg bg-primary text-white rounded-circle d-flex align-items-center justify-content-center mx-auto mb-3 shadow-sm"
                                ...
                                <div class="text-start px-3 mt-3">
                                    ...
                                    <p class="fw-bold">{{ \Carbon\Carbon::parse($t->tanggal_masuk)->translatedFormat('d F Y') }}
                                    </p>
                                </div>
                            </div>

                            <div class="col-md-8">
                                <h6 class="text-uppercase text-muted small fw-bold mb-3 border-bottom pb-2">Informasi Pribadi
                                </h6>
                                <div class="row mb-3">
                                    <div class="col-6 mb-3"><label class="small text-muted d-block">NIK</label><span
                                            class="fw-bold text-dark">{{ $t->nik }}</span></div>
                                    <div class="col-6 mb-3"><label class="small text-muted d-block">Jenis Kelamin</label><span
                                            class="fw-bold text-dark">{{ $t->jenis_kelamin }}</span></div>
                                    <div class="col-6 mb-3"><label class="small text-muted d-block">Tempat, Tgl
                                            Lahir</label><span class="fw-bold text-dark">{{ $t->tempat_lahir }},
                                            {{ \Carbon\Carbon::parse($t->tanggal_lahir)->format('d/m/Y') }}</span></div>
                                    <div class="col-6 mb-3"><label class="small text-muted d-block">Agama</label><span
                                            class="fw-bold text-dark">{{ $t->agama }}</span></div>
                                </div>

                                <h6 class="text-uppercase text-muted small fw-bold mb-3 border-bottom pb-2 pt-2">Kontak & Alamat
                                </h6>
                                <div class="row">
                                    <div class="col-6 mb-3"><label class="small text-muted d-block">Email</label><span
                                            class="fw-bold text-dark">{{ $t->email }}</span></div>
                                    <div class="col-6 mb-3"><label class="small text-muted d-block">No. Telepon</label><span
                                            class="fw-bold text-dark">{{ $t->nomor_telepon }}</span></div>
                                    <div class="col-12"><label class="small text-muted d-block">Alamat Lengkap</label><span
                                            class="fw-bold text-dark">{{ $t->alamat }}, {{ $t->kelurahan }},
                                            {{ $t->kecamatan }}, {{ $t->kabupaten }}, {{ $t->provinsi }} -
                                            {{ $t->kode_pos }}</span></div>
                                </div>
                            </div>
                        </div>
    ...
    @foreach($teachers as $t)
        <div class="modal fade" id="editTeacherModal{{ $t->id }}" tabindex="-1" data-bs-backdrop="static">
            <div class="modal-dialog modal-lg modal-dialog-scrollable">
                <form action="{{ route('guru.data.teachers.update', $t->id) }}" method="POST"
                    ...
                    <div class="modal-body p-4 bg-light">
                        ...
                        <ul class="nav nav-tabs mb-3" id="editTab{{ $t->id }}" role="tablist">
                            <li class="nav-item" role="presentation">
                                <button class="nav-link active fw-bold" id="profile-tab{{ $t->id }}" data-bs-toggle="tab"
                                    data-bs-target="#profile{{ $t->id }}" type="button" role="tab">📂 Data Profil</button>
                            </li>
                            <li class="nav-item" role="presentation">
                                <button class="nav-link fw-bold text-danger" id="account-tab{{ $t->id }}" data-bs-toggle="tab"
                                    data-bs-target="#account{{ $t->id }}" type="button" role="tab">🔒 Akun Login &
                                    Password</button>
                            </li>
                        </ul>
                        ...
                        <div class="tab-content" id="editTabContent{{ $t->id }}">
                            <div class="tab-pane fade show active" id="profile{{ $t->id }}" role="tabpanel">
                                ...
                                <div class="card border-0 shadow-sm mb-3">
                                    <div class="card-body">
                                        ...
                                        <div class="row g-3">
                                            <div class="col-md-6">
                                                <label class="form-label small fw-bold">NIP <span
                                            ...
                                            <div class="col-md-6">
                                                <label class="form-label small fw-bold">NIK <span
                                                        class="text-danger">*</span></label>
                                                <input type="number" name="nik" class="form-control" value="{{ $t->nik }}"
                                                    required>
                                            </div>
                                            <div class="col-md-6">
                                                <label class="form-label small fw-bold">Email <span
                                                        class="text-danger">*</span></label>
                                        ...
                                        <div class="row g-3">
                                            ...
                                            <div class="col-md-4">
                                                ...
                                                <input type="text" name="tempat_lahir" class="form-control"
                                                    value="{{ $t->tempat_lahir }}" required>
                                            </div>
                                            <div class="col-md-4">
                                                <label class="form-label small fw-bold">Tanggal Lahir <span
                                                        class="text-danger">*</span></label>
                                                <input type="date" name="tanggal_lahir" class="form-control"
                                                    value="{{ $t->tanggal_lahir }}" required>
                                            </div>
                                            <div class="col-md-6">
                                                <label class="form-label small fw-bold">Agama <span
                                                        class="text-danger">*</span></label>
                                                <select name="agama" class="form-select" required>
                                                    @foreach(['Islam', 'Kristen', 'Katolik', 'Hindu', 'Buddha', 'Konghucu'] as $agama)
                                                        <option value="{{ $agama }}" {{ $t->agama == $agama ? 'selected' : '' }}>
                                                            {{ $agama }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="col-md-6">
                                                ...
                                                <input type="text" name="nomor_telepon" class="form-control"
                                                    value="{{ $t->nomor_telepon }}" required>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="card border-0 shadow-sm mb-3">
                                    <div class="card-body">
                                        ...
                                        <div class="row g-3">
                                            <div class="col-12">
                                                <label class="form-label small fw-bold">Alamat Jalan <span
                                                        class="text-danger">*</span></label>
                                                <textarea name="alamat" class="form-control" rows="2"
                                                    required>{{ $t->alamat }}</textarea>
                                            </div>
                                            <div class="col-md-6">
                                                <label class="form-label small fw-bold">Provinsi <span
                                                        class="text-danger">*</span></label>
                                                <input type="text" name="provinsi" class="form-control"
                                                    value="{{ $t->provinsi }}" required>
                                            </div>
                                            <div class="col-md-6">
                                                <label class="form-label small fw-bold">Kabupaten/Kota <span
                                                        class="text-danger">*</span></label>
                                                <input type="text" name="kabupaten" class="form-control"
                                                    value="{{ $t->kabupaten }}" required>
                                            </div>
                                            <div class="col-md-6">
                                                <label class="form-label small fw-bold">Kecamatan <span
                                                        class="text-danger">*</span></label>
                                                <input type="text" name="kecamatan" class="form-control"
                                                    value="{{ $t->kecamatan }}" required>
                                            </div>
                                            <div class="col-md-6">
                                                <label class="form-label small fw-bold">Kelurahan <span
                                                        class="text-danger">*</span></label>
                                                <input type="text" name="kelurahan" class="form-control"
                                                    value="{{ $t->kelurahan }}" required>
                                            </div>
                                            <div class="col-md-6">
                                            ...
                                            <div class="col-md-6">
                                                <label class="form-label small fw-bold">Kode Pos <span
                                                        class="text-danger">*</span></label>
                                                <input type="number" name="kode_pos" class="form-control"
                                                    value="{{ $t->kode_pos }}" required>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="card border-0 shadow-sm">
                                    <div class="card-body">
                                        ...
                                        <div class="row g-3">
                                            <div class="col-md-4">
                                                <label class="form-label small fw-bold">Jabatan <span
                                                        class="text-danger">*</span></label>
                                                <select name="jabatan" class="form-select" required>
                                                    <option value="" disabled>-- Pilih Jabatan --</option>
                                                    <option value="Wali Kelas" {{ $t->jabatan == 'Wali Kelas' ? 'selected' : '' }}>Wali Kelas</option>
                                                    <option value="Guru Olahraga" {{ $t->jabatan == 'Guru Olahraga' ? 'selected' : '' }}>Guru Olahraga</option>
                                                    <option value="Guru Mengaji" {{ $t->jabatan == 'Guru Mengaji' ? 'selected' : '' }}>Guru Mengaji</option>
                                                </select>
                                            </div>
                                            <div class="col-md-4">
                                                <label class="form-label small fw-bold">Status <span
                                                        class="text-danger">*</span></label>
                                                <select name="status" class="form-select" required>
                                                    <option value="Aktif" {{ $t->status == 'Aktif' ? 'selected' : '' }}>Aktif
                                                    </option>
                                                    <option value="Cuti" {{ $t->status == 'Cuti' ? 'selected' : '' }}>Cuti
                                                    </option>
                                                    <option value="Pindah" {{ $t->status == 'Pindah' ? 'selected' : '' }}>Pindah
                                                    </option>
                                                    <option value="Pensiun" {{ $t->status == 'Pensiun' ? 'selected' : '' }}>
                                                        Pensiun</option>
                                                </select>
                                            </div>
                                            <div class="col-md-4">
                                                <label class="form-label small fw-bold">Tanggal Masuk <span
                                                        class="text-danger">*</span></label>
                                                <input type="date" name="tanggal_masuk" class="form-control"
                                                    value="{{ $t->tanggal_masuk }}" required>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="tab-pane fade" id="account{{ $t->id }}" role="tabpanel">
                                <div class="card border-0 shadow-sm bg-white">
                                    <div class="card-body">
                                        <h6 class="fw-bold text-danger mb-3"><i class="fas fa-lock me-2"></i>Pengaturan Keamanan
                                            Akun</h6>
                                        ...
                                        <div class="row g-3">
                                            <div class="col-12">
                                                ...
                                                <select name="is_active" class="form-select">
                                                    <option value="1" {{ $t->user && $t->user->is_active ? 'selected' : '' }}>✅
                                                        Aktif (Bisa Login)</option>
                                                    <option value="0" {{ $t->user && !$t->user->is_active ? 'selected' : '' }}>❌
                                                        Non-Aktif (Dilarang Login)</option>
                                                </select>
                                                <div class="form-text small text-muted">Jika diset Non-Aktif, guru ini tidak
                                                    akan bisa login ke sistem meskipun data profilnya masih ada.</div>
                                            </div>

                                            <div class="col-12">
                                                <div class="alert alert-warning border-0 bg-warning bg-opacity-10 small">
                                                    <i class="fas fa-key me-1"></i> <b>Reset Password</b>
                                                    <br>Masukkan password baru di bawah ini hanya jika Anda ingin mengubah
                                                    password guru tersebut.
                                                </div>
            ...
            <form action="{{ route('guru.data.teachers.store') }}" method="POST" class="modal-content border-0 shadow-lg">
                ...
                <div class="modal-body p-4 bg-light">
                    ...
                    <div class="card border-0 shadow-sm mb-3">
                        <div class="card-body">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    <label class="form-label small fw-bold">Email (Aktif) <span
                                            class="text-danger">*</span></label>
                    ...
                    <div class="alert alert-info small mt-3 py-2 border-0 bg-opacity-10 bg-info text-info">
                        <i class="fas fa-info-circle me-1"></i> Akun login akan dibuat otomatis dengan username <b>NIP</b>
                        dan password default: <b>password123</b>
                    </div>
                </div>
                <div class="modal-footer border-0 bg-white">
                    <button type="button" class="btn btn-secondary rounded-pill px-4" data-bs-dismiss="modal">Batal</button>
