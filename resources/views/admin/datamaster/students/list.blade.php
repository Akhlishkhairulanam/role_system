<x-app-layout>
    ...
    <div class="container-fluid px-4 mt-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <!-- UBAHAN: Menambahkan Kotak Pencarian di Samping Tombol Tambah -->
            <div class="d-flex gap-3 align-items-center">
                <div class="input-group shadow-sm">
                    ...
                    <input type="text" id="searchInClass" class="form-control border-start-0 ps-2"
                        placeholder="Cari nama atau NISN..." style="width: 250px;">
                </div>

                <button class="btn btn-primary rounded-pill shadow-sm px-4 text-nowrap" onclick="openAddModal()">
                                ...
                                <tr class="student-row">
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <div class="avatar-circle bg-primary bg-opacity-10 text-primary me-3 d-flex align-items-center justify-content-center rounded-circle fw-bold"
                                    ...
                                    <td>
                                        <div class="small text-dark"><i class="fas fa-phone me-1 text-muted"></i>
                                            {{ $s->nomor_telepon ?? '-' }}</div>
                                    ...
                                    <td class="text-end px-4">
                                        <div class="btn-group">
                                            <a href="{{ route('guru.data.students.rapor', $s->id) }}"
                                                class="btn btn-sm btn-light text-info border-0 rounded-circle me-1"
                                                title="Lihat Rapor">
                                            ...
                                            <form id="delete-student-{{ $s->id }}"
                                                action="{{ route('guru.data.students.destroy', $s->id) }}" method="POST"
                                                class="d-none">
                                                @csrf @method('DELETE')
                                            </form>
                                        </div>
            ...
            <form id="studentForm" method="POST" class="modal-content border-0 shadow-lg">
                ...
                <div class="modal-body p-0">
                    <ul class="nav nav-tabs nav-justified bg-light" id="studentTabs" role="tablist">
                        <li class="nav-item">
                            <button class="nav-link active" data-bs-toggle="tab" data-bs-target="#tab-pribadi"
                                type="button">1. Data Pribadi</button>
                        </li>
                        <li class="nav-item">
                            <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tab-alamat" type="button">2.
                                Alamat</button>
                        </li>
                        <li class="nav-item">
                            <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tab-ortu" type="button">3.
                                Orang Tua</button>
                        </li>
                    </ul>
                    ...
                    <div class="tab-content p-4">
                        ...
                        <div class="tab-pane fade show active" id="tab-pribadi">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    <label class="form-label small fw-bold">NISN (Username Login) <span
                                            class="text-danger">*</span></label>
                        ...
                        <div class="tab-pane fade" id="tab-ortu">
                            <div class="alert alert-info border-0 small bg-info bg-opacity-10 text-info-emphasis mb-3">
                                <i class="fas fa-info-circle me-1"></i> Tautkan akun orang tua agar data siswa muncul di
                                dashboard mereka.
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <select name="parent_user_id" id="parent_user_id"
                                    class="form-select border-primary bg-light">
                            ...
                            <div id="resetPasswordContainer" class="mt-4 pt-3 border-top d-none">
                                <div class="form-check form-switch">
                                    ...
                                    <label class="form-check-label text-danger fw-bold" for="reset_password">Reset
                                        Password Siswa ke Default ('password123')</label>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    @push('scripts')
        <script>
            ...
            // Script Pencarian di dalam Kelas
            document.getElementById('searchInClass').addEventListener('keyup', function () {
                let filter = this.value.toLowerCase().trim();
                let rows = document.querySelectorAll('#studentTableBody .student-row');
                let emptyState = document.getElementById('emptyState');
                let visibleCount = 0;

                rows.forEach(row => {
                    // row.textContent akan mengambil semua teks di dalam baris (Nama, NISN, Alamat, dll)
                ...
                if (visibleCount === 0 && filter !== '' && rows.length > 0) {
                    if (!noResultRow) {
                        ...
                        tbody.insertAdjacentHTML('beforeend', `<tr id="noResultRow"><td colspan="6" class="text-center py-4 text-muted fst-italic">Siswa yang dicari tidak ditemukan di kelas ini.</td></tr>`);
                    }
                } else {
                    if (noResultRow) {
                        noResultRow.remove();
