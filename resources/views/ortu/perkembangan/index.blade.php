            ...
            <div class="card-body p-4 position-relative z-1">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <p class="mb-0 opacity-75">NISN: {{ $anak->nisn }} • Kelas
            ...
            <div class="col-lg-8">
                    ...
                    <div class="col-6 col-md-3">
                        <div class="card border-0 rounded-4 shadow-sm bg-success bg-opacity-10 h-100">
                            <div class="card-body text-center p-3">
                                ...
                                <small class="text-muted fw-bold text-uppercase" style="font-size: 0.7rem;">Hadir
                                    (H)</small>
                            </div>
                        </div>
                    </div>
                    <div class="col-6 col-md-3">
                        <div class="card border-0 rounded-4 shadow-sm bg-info bg-opacity-10 h-100">
                            <div class="card-body text-center p-3">
                                ...
                                <small class="text-muted fw-bold text-uppercase" style="font-size: 0.7rem;">Izin
                                    (I)</small>
                            </div>
                        </div>
                    </div>
                    <div class="col-6 col-md-3">
                        <div class="card border-0 rounded-4 shadow-sm bg-warning bg-opacity-10 h-100">
                            <div class="card-body text-center p-3">
                                ...
                                <small class="text-muted fw-bold text-uppercase" style="font-size: 0.7rem;">Sakit
                                    (S)</small>
                            </div>
                        </div>
                    </div>
                    <div class="col-6 col-md-3">
                        <div class="card border-0 rounded-4 shadow-sm bg-danger bg-opacity-10 h-100">
                            <div class="card-body text-center p-3">
                                ...
                                <small class="text-muted fw-bold text-uppercase" style="font-size: 0.7rem;">Alpha
                                    (A)</small>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card border-0 rounded-4 shadow-sm mb-4">
                    <div class="card-header bg-white border-0 pt-4 px-4">
                        <h6 class="fw-bold text-dark"><i class="fas fa-chart-pie text-primary me-2"></i>Persentase
                            Kehadiran</h6>
                    </div>
                    <div class="card-body p-4">
                        ...
                        <div class="progress rounded-pill" style="height: 25px;">
                            <div class="progress-bar bg-gradient-primary progress-bar-striped progress-bar-animated"
                                role="progressbar" style="width: {{ $persentase_kehadiran }}%">
                            </div>
                        </div>
                ...
                <div class="card border-0 rounded-4 shadow-sm">
                    <div class="card-header bg-white border-0 pt-4 px-4">
                        <h6 class="fw-bold text-dark"><i class="fas fa-history text-secondary me-2"></i>Riwayat 5
                            Absensi Terakhir</h6>
                    </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover align-middle mb-0">
                                ...
                                <tbody>
                                    @forelse($riwayat_absensi as $absen)
                                        <tr>
                                            <td class="ps-4 fw-bold text-dark">
                                                {{ \Carbon\Carbon::parse($absen->date)->format('d M Y') }}</td>
                                            <td>
                                                @if ($absen->teacher_allocation)
                                                    <span
                                                        class="fw-bold text-primary">{{ $absen->teacher_allocation->subject->nama_mapel ?? 'Mapel Umum' }}</span>
                                                    <div class="small text-muted">
                                                        {{ $absen->teacher_allocation->teacher->nama_lengkap ?? '-' }}
                                                    </div>
                                                @else
                                            ...
                                            <td>
                                                ...
                                                @elseif($absen->status == 'I')
                                                    <span class="badge bg-info rounded-pill px-3">Izin</span>
                                                @elseif($absen->status == 'S')
                                                    <span class="badge bg-warning rounded-pill px-3">Sakit</span>
                                                @elseif($absen->status == 'A')
                                                    <span class="badge bg-danger rounded-pill px-3">Alpha</span>
                                                @else
                                                    <span
                                                        class="badge bg-secondary rounded-pill px-3">{{ $absen->status }}</span>
                                                @endif
                                            </td>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="4" class="text-center py-4 text-muted">Belum ada data
                                                absensi.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
            ...
            <div class="col-lg-4">
                <div class="card border-0 rounded-4 shadow-sm bg-white mb-4">
                                ...
                                <li
                                    ...
                                    <div>
                                        <h6 class="fw-bold mb-0 text-dark">
                                            {{ $nilai->teacher_allocation->subject->nama_mapel ?? 'Mapel' }}</h6>
                                        <small class="text-muted">{{ $nilai->type }}</small>
                                    </div>
                                    <span class="badge bg-primary fs-6 rounded-3">{{ $nilai->score }}</span>
                                </li>
                            @empty
                                <li class="list-group-item px-4 py-3 text-center text-muted small">Belum ada nilai
                                    masuk.</li>
                            @endforelse
                        </ul>
                    </div>
                    <div class="card-footer bg-white border-0 pb-4 px-4">
                        <a href="{{ route('ortu.nilai') }}"
                            class="btn btn-outline-primary w-100 rounded-pill btn-sm">Lihat Semua Nilai</a>
                    </div>
                </div>
                ...
                <div class="card border-0 rounded-4 shadow-sm bg-dark text-white overflow-hidden">
                    <div class="card-body p-4 position-relative">
                        ...
                        <a href="{{ route('ortu.rapor') }}"
                            class="btn btn-light text-dark fw-bold rounded-pill px-4 shadow-sm">
                            Buka E-Rapor <i class="fas fa-arrow-right ms-1"></i>
                        </a>
                        <i class="fas fa-trophy position-absolute text-warning opacity-25"
                            style="top: -10px; right: -10px; font-size: 5rem;"></i>
                    </div>
                </div>
            </div>
        </div>
