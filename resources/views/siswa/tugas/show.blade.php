        ...
        <div class="row">
                ...
                <div class="card border-0 shadow-sm rounded-4 h-100">
                    <div class="card-body p-4">
                        <div class="d-flex align-items-center mb-3">
                            ...
                            {{-- FIX KOLOM TYPE --}}
                            <span
                                class="badge {{ $assignment->type == 'online' ? 'bg-info' : 'bg-warning' }} bg-opacity-10 {{ $assignment->type == 'online' ? 'text-info' : 'text-dark' }} px-3 py-2 rounded-pill">
                                <i
                                    class="fas {{ $assignment->type == 'online' ? 'fa-globe' : 'fa-hand-holding' }} me-1"></i>
                        ...
                        <div class="p-3 bg-light rounded-3 mb-4 text-secondary"
                        ...
                        @if ($assignment->file_path)
                            <div class="mb-4">
                                ...
                                <a href="{{ route('siswa.tugas.download_attachment', $assignment->id) }}"
                        ...
                        <div class="row text-center border-top pt-3">
                            ...
                            <div class="col-6">
                                ...
                                <span class="fw-bold text-dark">
                                    ...
                                    {{ $assignment->teacher->nama_lengkap ?? ($assignment->teacher->name ?? 'Guru') }}
                                </span>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-lg-4">
                ...
                @if ($submission && $submission->grade !== null)
                    <div
                        class="card border-0 shadow-sm rounded-4 overflow-hidden mb-4 border-start border-primary border-4">
                        <div class="card-body p-4 text-center">
                            <small class="text-muted d-block mb-1 text-uppercase fw-bold" style="letter-spacing: 1px;">Nilai
                                Hasil Kerja</small>
                            <h1
                                class="display-4 fw-bold {{ $submission->grade >= 75 ? 'text-success' : 'text-danger' }} mb-0">
                                {{ $submission->grade }}
                            </h1>
                            <hr class="my-3">
                            <div class="text-start">
                                <label class="small fw-bold text-primary"><i class="fas fa-comment-dots me-1"></i> Feedback
                                    Guru:</label>
                    ...
                    <div class="card-body p-4 pt-0">
                        ...
                        @if ($submission)
                            <div class="text-center py-3">
                                <div class="bg-success bg-opacity-10 text-success rounded-4 p-3 mb-3">
                                    ...
                                    <small
                                        class="opacity-75">{{ $submission->updated_at->translatedFormat('d M, H:i') }}</small>
                                </div>

                                @if ($submission->file_path)
                                    <a href="{{ route('siswa.tugas.download_submission', $submission->id) }}"
                                ...
                                @if ($submission->grade === null && now() < $assignment->deadline)
                                    <button class="btn btn-light btn-sm w-100 rounded-pill text-muted shadow-sm"
                                        data-bs-toggle="collapse" data-bs-target="#formSubmission">
                        ...
                        <div class="collapse {{ !$submission ? 'show' : '' }} mt-2" id="formSubmission">
                            @if (now() < $assignment->deadline)
                                ...
                                <form action="{{ route('siswa.tugas.submit', $assignment->id) }}" method="POST"
                                    enctype="multipart/form-data">
                                    @csrf
                                    {{-- FIX KOLOM TYPE --}}
                                    @if ($assignment->type == 'online')
                                        <div class="mb-3">
                                            <label class="small fw-bold mb-1">Pilih File
                                                {{ $submission ? '(Ganti File)' : 'Jawaban' }}</label>
                                            <input type="file" name="file"
                                                class="form-control form-control-sm rounded-3">
                                            @if (!$submission)
                                                <small class="text-danger" style="font-size: 9px;">*Wajib diisi
                                                    (PDF/JPG/DOCX)</small>
                                            @endif
                                        </div>
                                    @else
                                        <div class="bg-light p-3 rounded-3 mb-3 text-center border">
                                            ...
                                            <p class="small mb-0 text-dark">Tugas ini dikumpulkan secara
                                                <strong>Offline</strong> di kelas.</p>
                                        </div>
                                    @endif
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <textarea name="notes" class="form-control form-control-sm rounded-3" rows="2"
                                            placeholder="Tulis catatan jika ada...">{{ $submission->notes ?? '' }}</textarea>
                                    </div>
                                ...
                                @if ($submission && $submission->grade === null)
                                    <form id="form-hapus-tugas" action="{{ route('siswa.tugas.delete', $assignment->id) }}"
                                        method="POST" class="d-none">
                                        @csrf
                                        @method('DELETE')
                                    </form>

                                    <button type="button" onclick="confirmCancelSubmission()"
                                        class="btn btn-link text-danger w-100 small text-decoration-none mt-2">
